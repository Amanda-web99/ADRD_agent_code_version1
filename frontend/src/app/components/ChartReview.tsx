...
const CAT_STYLE: Record<HighlightCategory, { bg: string; text: string; label: string }> = {
  ...
  lab:        { bg: "bg-sky-100",    text: "text-sky-900",    label: "Lab"            },
  medication: { bg: "bg-green-100",  text: "text-green-900",  label: "Medications"    },
  radiology:  { bg: "bg-orange-100", text: "text-orange-900", label: "Radiology"      },
...
const SECTIONS = [
  { key: "history",   label: "History",           dot: "bg-yellow-500" },
  { key: "lab",       label: "Lab",               dot: "bg-sky-500"    },
  { key: "medication",label: "Medication",        dot: "bg-green-500"  },
  { key: "radiology", label: "Radiology",         dot: "bg-orange-500" },
  { key: "cognitive", label: "Cognitive Tests",   dot: "bg-purple-500" },
];
...
export default function ChartReview() {
  ...
  const scrollTo = (key: string) => {
    setActiveNav(key);
    if (key === "history" || key === "lab" || key === "medication" || key === "radiology" || key === "cognitive") {
      const sectionKey = key as HighlightCategory;
      setFilterTab(sectionKey);

      const firstEvidenceInSection = evidenceForPanel
        .filter((ev) => ev.category === sectionKey)
        .sort((a, b) => {
          const aSpan = analysis.spans.find((s) => s.id === a.spanId);
          const bSpan = analysis.spans.find((s) => s.id === b.spanId);
          const aStart = aSpan?.start ?? Number.MAX_SAFE_INTEGER;
          const bStart = bSpan?.start ?? Number.MAX_SAFE_INTEGER;
          return aStart - bStart;
        })[0];

      const fallbackSpan = analysis.spans
        .filter((span) => span.category === sectionKey)
        .sort((a, b) => a.start - b.start)[0];

      const targetSpanId = firstEvidenceInSection?.spanId || fallbackSpan?.id || null;

      setActiveEvId(firstEvidenceInSection?.id ?? null);
      setActiveSpanId(targetSpanId);

      setTimeout(() => {
        if (targetSpanId) {
          const el = spanRefs.current.get(targetSpanId);
          if (el) {
  ...
  const dx = editDx;
  const evidenceBySpanId = new Map(analysis.evidence.map((ev) => [ev.spanId, ev]));
  const evidenceForPanel = analysis.spans
    .slice()
    .sort((a, b) => a.start - b.start)
    .map((span, idx) => {
      const existing = evidenceBySpanId.get(span.id);
      if (existing) return existing;
      return {
        id: 100000 + idx,
        type: `${CAT_STYLE[span.category].label} Evidence`,
  ...
  return (
    <div className="h-screen flex flex-col bg-white overflow-hidden">
      ...
      {/* ════════════ BODY (3 columns) ════════════ */}
      <ResizablePanelGroup direction="horizontal" className="flex-1 w-full min-w-0 overflow-hidden">

        {/* ── LEFT: Section Navigator ── */}
        <ResizablePanel defaultSize={18} minSize={12} maxSize={30} className="min-w-0">
        <aside className="h-full w-full min-w-0 border-r border-gray-200 bg-white overflow-y-auto">
        ...
        <ResizableHandle withHandle className="bg-gray-200/80 hover:bg-gray-300 transition-colors" />

        {/* ── MIDDLE: Clinical Note ── */}
        <ResizablePanel defaultSize={52} minSize={30} className="min-w-0">
        <main
          ref={noteScrollRef}
          className="h-full w-full min-w-0 overflow-y-auto bg-white"
        >
          {/* Sticky filter bar */}
          ...
          <div className="px-6 py-5">
            ...
            {/* ─ Timeline ─ */}
            <section id="sec-timeline" className="mb-8">
        ...
        <aside
          ref={evPanelRef}
          className="h-full w-full min-w-0 border-l border-gray-200 bg-white overflow-y-auto"
        >
          {/* panel header */}
          ...
          <div className="px-5 py-4 space-y-5">
            ...
            <div>
              <p className="text-sm font-semibold text-gray-900 mb-3">
                Evidence ({evidenceForPanel.length})
              </p>

              <div className="space-y-0">
                {evidenceForPanel.map((ev, idx) => {
