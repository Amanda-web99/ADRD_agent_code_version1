...
export default function FileUpload() {
  ...
  const processExcelFile = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      const patientData: PatientData[] = jsonData
        .map((row: any) => ({
          patientId: String(row["Patient ID"] || row["patientId"] || row["patient_id"] || "").trim(),
          notes: String(row["Notes"] || row["notes"] || row["Clinical Notes"] || "").trim(),
          admissionDate: String(row["Admission Date"] || row["admissionDate"] || row["admission_date"] || "").trim(),
        }))
        .filter((item: PatientData) => item.notes.length > 0);
  ...
  const handleAnalyze = async () => {
    if (patients.length === 0) {
      alert("No valid notes found. Please upload an Excel file with non-empty Notes/Clinical Notes column.");
      return;
    }
    ...
    try {
      ...
      if (analysisResults.length === 0) {
        throw new Error("No analyzable patient notes found in uploaded file.");
      }
